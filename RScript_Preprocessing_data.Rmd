...
This script provides you with a pipeline for preprocessing one data file of the Danish Gigaword Corpus (Strømberg-Derczynski et al., 2020). The preprocessing includes standard natural language processing steps such as tokenization and lemmatization. Hence, this script displays what has been done with the entire Danish Gigaword Corpus demonstrated on a single text-file. Running this script will provide you with a preprocessed csv-file of pure text.
...
```{r}
library(pacman)
pacman::p_load(udpipe, stopwords, dplyr)
```

# Load data
```{r}
data <- read.delim("/path/to/data", header=FALSE, quote="", fill=FALSE) 
```

# Load Danish UDpipe Model for annotation
```{r}
# Get danish model
...
Create the right format (CSV-file) for later use 
```{r}
...
Generating a csv-file of the preprocessed data
```{r}
write.csv(x2, file = "preprocessed_data.csv", row.names = FALSE)
```